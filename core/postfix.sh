...
trap "{ echo Stopping postfix; /usr/sbin/postmulti -i - -x ${DAEMON} quiet-stop exit 0; }" SIGSTOP SIGINT EXIT SIGTERM
...
configure_instance() {
    POSTCONF="postmulti -i - -x postconf"
...
/usr/sbin/postmulti -i - -x ${DAEMON} quiet-quick-start
